...
# 4. On parcourt les nb_tours plus courtes distances pour construire les regroupements
nb_tours = 1000
...
# 5. On prend le nombre de box dans les trois plus gros regroupements
regroupements_uniques = set(map_regroupements.values())
regroupements_tries = sorted(regroupements_uniques, key=lambda r: len(r.boxes), reverse=True)


score = 1
for i in range(min(3, len(regroupements_tries))):
    score *= len(regroupements_tries[i].boxes)
