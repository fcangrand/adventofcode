...
let maxResult = 0;
let arrayResult = [];
const maxAmplifiers = 5;

function ContestResponse() {
    const tab = input[0].split(',').map(x => +x);
	calculatePath(tab, 0, [], 0);
    console.log('sortie ', arrayResult + ' max: ' + maxResult);
}

function calculatePath(p_tab, p_input2, p_alreadyChecked, depth) {
	for (let i = 0; i < maxAmplifiers; i++) {
		if (p_alreadyChecked.indexOf(i) === -1) {
			...
			calculatePath(p_tab, result, alreadyChecked, depth+1);
			if (depth === (maxAmplifiers-1) && result > maxResult) {
				maxResult = result;
